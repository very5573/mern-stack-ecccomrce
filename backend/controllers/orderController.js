...
export const updateOrder = async (req, res) => {
  ...
  try {
    ...
    const allowedTransitions = {
      Processing: ["Soon", "Shipped", "Cancelled"],
      Soon: ["Shipped", "Cancelled"],
      Shipped: ["Delivered"],
      Delivered: [],
      Cancelled: [],
    };
    ...
    for (const orderId of orderIds) {
       ...
       * (Only if stock was already deducted in createOrder)
       */
      if (
        newStatus === "Cancelled" &&
        prevStatus !== "Cancelled" &&
        prevStatus !== "Delivered" &&
        order.stockUpdated // 👈 stock createOrder me minus hua tha
      ) {
      ...
      /**
       * 🚫 NO STOCK CHANGE AFTER DELIVERY
       */
