...
const DashboardLayout = ({ children }) => {
    ...
    <ProtectedRoute allowedRoles={["admin"]}>
      <div className={`flex min-h-screen w-full ${darkMode ? "dark" : ""}`}>
        ...
        <motion.aside
          ...
          className={`fixed left-0 top-0 z-50 h-full flex flex-col shadow-xl
                        ...
                        >
                          {item.children.map((child) => (
                            <li key={child.name}>
                              <Link
                                href={child.link}
                                className={`block rounded px-2 py-2 text-sm font-medium transition
                                  ${
                                    pathname === child.link
                                      ? darkMode
                                        ? "bg-gray-700"
          ...
          {(sidebarOpen || hovered) && (
            ...
            >
              <button
                ...
                className={`w-full rounded px-3 py-1 text-sm font-medium transition
                  ${darkMode ? "hover:bg-gray-700" : "hover:bg-gray-200"}`}
              >
  ...
  className={`
    flex-1 flex flex-col min-h-screen   /* full screen height */
    ${sidebarOpen || hovered ? "ml-64" : "ml-20"}
    ${darkMode ? "bg-gray-800 text-white" : ""}
  `}
>
  {/* CHILDREN WRAPPER */}
  <div className="flex-1 flex flex-col -mt-16 justify-start">
    {children}
  </div>
</motion.main>
